...
const DECISION_KEYWORDS =
  /为什么|为何|原因|方案|决定|决策|选择|不用|改成|换成|放弃|权衡|取舍|考虑过|试过|坑|问题|瓶颈|优化|重构|架构|设计|模式|策略|实现|方式|why|because|reason|decided|decision|instead|tradeoff|trade-off|approach|pattern|constraint|alternative|evaluate|compared/i;
...
function checkMemoryUsage() {
  ...
  if (rssMB > 1024) { // 超过 1GB
    console.log(`⚠️  Memory usage high: ${rssMB} MB RSS`);
    return { warning: true, rssMB, heapUsedMB };
  }
  return { warning: false, rssMB, heapUsedMB };
}
...
async function main() {
  ...
  console.log(`   Memory: ${initialMemory.heapUsedMB} MB heap / ${initialMemory.rssMB} MB RSS\n`);

  if (!fs.existsSync(SESSION_DIR)) {
    process.stderr.write(`❌ Session directory not found: ${SESSION_DIR}\n`);
    process.stderr.write('   Please ensure Claude Code CLI is installed and has been used.\n');
    process.stderr.write('   The directory ~/.claude/projects/ should exist.\n');

    // 降级：创建空文件并提示用户
    console.log('\n⚠️  Fallback: Creating empty extracted_decisions.md');
    console.log('   Interview preparation will rely on code analysis only.\n');
  ...
  if (selectedFiles.length === 0) {
    console.log(`⚠️  No session files found in the last ${DAYS_LIMIT} days.`);
    console.log('   This is normal if you haven\'t used Claude Code recently.');
    console.log('   Interview preparation will rely on code analysis only.\n');

    // 降级：创建空文件并提示
    writeOutput([], OUTPUT_FILE);
    console.log(`✅ Created empty ${OUTPUT_FILE}`);
    console.log('   You can still generate interview questions from your codebase.\n');
    process.exit(0);
  }
  ...
  for (const filepath of selectedFiles) {
    ...
    // 检查单个文件大小，超过 100MB 跳过
    if (stat.size > 100 * 1024 * 1024) {
      console.log(`  ⏭️  Skipping ${path.basename(filepath)} (${sizeMb} MB) - too large`);
      skippedFiles++;
      continue;
    }

    process.stdout.write(`  📖 Loading ${path.basename(filepath)} (${sizeMb} MB)...\n`);

    try {
      const fileMessages = await loadJsonl(filepath);
      allMessages = allMessages.concat(fileMessages);
      processedFiles++;

      // 每处理 5 个文件检查一次内存
      if (processedFiles % 5 === 0) {
        const memStatus = checkMemoryUsage();
        if (memStatus.warning) {
          console.log(`\n⚠️  Memory warning: ${memStatus.rssMB} MB RSS`);
          console.log('   Consider using --max-files to limit processing.\n');
        }
      }
    } catch (err) {
      console.log(`  ⚠️  Error loading ${path.basename(filepath)}: ${err.message}`);
      skippedFiles++;
      continue;
    }
  }

  console.log(`\n📊 Loaded ${allMessages.length} messages from ${processedFiles} files`);
  if (skippedFiles > 0) {
    console.log(`   Skipped ${skippedFiles} files (too large or errors)`);
  }
  console.log();
  ...
  const summary = {
    status: messages.length > 0 ? 'success' : 'fallback',
    session_dir: SESSION_DIR,
    files_processed: processedFiles,
    files_skipped: skippedFiles,
    messages_extracted: messages.length,
    output_file: OUTPUT_FILE,
    output_size_kb: Math.round(fs.statSync(OUTPUT_FILE).size / 1024),
    memory_used_mb: finalMemory.rssMB,
    warnings: {
      no_decisions: messages.length === 0,
      high_memory: finalMemory.warning
    }
  };

  console.log('\n------------------------------------------------------------');
  console.log('  JSON Summary (for Claude parsing)');
  console.log('------------------------------------------------------------');
  ...
  console.log('   bash code_analyzer.sh /path/to/your/project');
  console.log('');
}

main().catch(err => {
  process.stderr.write(`\n❌ Session extraction failed: ${err.message}\n`);

  // 提供具体的错误分类和解决方案
  if (err.message.includes('EACCES') || err.message.includes('permission')) {
    process.stderr.write('\n🔧 This is a permission error.\n');
    process.stderr.write('   Try: chmod -R 755 ~/.claude/projects\n');
  } else if (err.message.includes('ENOSPC') || err.message.includes('space')) {
    process.stderr.write('\n🔧 Disk space is low.\n');
    process.stderr.write('   Try: --max-files 5 to process fewer files\n');
  } else if (err.message.includes('JSON')) {
    process.stderr.write('\n🔧 Some session files may be corrupted.\n');
    process.stderr.write('   The extractor skipped invalid entries.\n');
  } else {
    process.stderr.write(`\n🔧 Unexpected error.\n`);
  ...
  try {
    writeOutput([], OUTPUT_FILE);
    process.stderr.write(`\n⚠️  Fallback: Created empty ${OUTPUT_FILE}\n`);
    process.stderr.write('   Interview prep will use code analysis only.\n');
    process.exit(0);
  } catch {
    process.exit(1);
  }
